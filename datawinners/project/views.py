...
def make_project_links(project, questionnaire_code, reporter_id=None):
    ...
    if project.state == ProjectState.TEST or project.state == ProjectState.ACTIVE:
        ...
        project_links['sender_registration_preview_link'] = reverse( sender_registration_form_preview,
                                                                     args=[project_id] )
...
def project_results(request, project_id=None, questionnaire_code=None):
    ...
    if request.method == "POST":
        ...
        for submission_id in submission_ids:
            ...
            submission.void( )
            ReportRouter( ).delete( get_organization( request ).org_id, submission.form_code,
                                    submission.data_record.id )
    ...
    if count:
        raw_data_list.extend(
            [[submission.destination, submission.source, submission.created, ugettext( str( submission.status ) ),
              ...
              submission.errors] + [submission.values.get( q.code.lower( ) ) for q in questionnaire.fields] for
             submission
             in submissions] )
    ...
    if request.method == 'POST':
        ...
        if len( imported_entities.keys( ) ):
            UserActivityLog( ).log( request, action="Imported Data Senders",
                                    detail=json.dumps(
                                        dict( {"Unique ID": "[%s]" % ", ".join( imported_entities.keys( ) )} ) ),
...
def edit_data_sender(request, project_id, reporter_id):
    ...
    if request.method == 'GET':
        ...
        form = ReporterRegistrationForm( initial={'project_id': project_id, 'name': reporter_entity.name,
                                                  ...
                                                  'location': location, 'geo_code': geo_code} )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid( ):
            try:
                ...
                web_player = WebPlayer( manager, LocationBridge( location_tree=get_location_tree( ),
                ...
                if response.success:
                    ...
                    for field in ["geo_code", "location", "mobile_number", "name"]:
                        if getattr( reporter_entity, field ) != form.cleaned_data.get(
